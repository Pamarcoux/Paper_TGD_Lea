...
##### Workflow ##### 
Workflow_malc_png <- magick::image_read(here('data/Workflow_malc_tgd.png'))
...
plot_depletion_UT_thaw <- ggplot(Df_depletion_UT_thaw, 
  ...
  scale_color_manual(values = palette_TDG_pastel) +
  stat_compare_means(method = "wilcox.test", p.adjust.method = "BH", aes(group = Purification), hide.ns = FALSE,
                     paired = FALSE,label = "p.signif", size = 3, vjust = 1, hjust = 0.5, label.x.npc = "middle") +
  theme_blood() +
  theme(strip.text = element_text(size = 8, face = "bold"),
        legend.position =  "none") +
...
plot_CD107a_non_purified <- ggplot(Df_cd107a_non_purified, aes(x = Condition, y = Per_CD107a)) +
  ...
  scale_color_manual(values = palette_TDG_pastel) +
  stat_compare_means(method = "wilcox.test", p.adjust.method = "BH", aes(group = Condition), hide.ns = FALSE,
                     paired = TRUE,label = "p.signif", size = 3, vjust = 1, hjust = 0.5, label.x.npc = "middle") +
  theme_blood() +
  ylim(0,50)+
  theme(strip.text = element_text(size = 8, face = "bold"),
...
#### Imagerie ####
Figure_imagerie_png <- magick::image_read(here('data/Figure_imagerie.png'))
...
ggsave(
  filename = here('outputs/Figures/Figure_Lea_3.png'),
  plot = Figure_3,
  device = "png",
